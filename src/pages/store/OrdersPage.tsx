...
const labels:Record<OrderStatus,string>={pending_payment:'Aguardando pagamento',paid:'Pago / confirmado',preparing:'Em preparo',ready:'Pronto',out_for_delivery:'Saiu para entrega',completed:'Concluído',cancelled:'Cancelado'};
...
const flow:OrderStatus[]=['paid','preparing','ready','out_for_delivery','completed'];
...
const stepIndex=(status:OrderStatus)=>flow.indexOf(status);
...
export function OrdersPage(){
 ...
 async function update(id:string,data:{status?:OrderStatus;merchantNotes?:string}){if(busyIds[id])return;setBusyIds(v=>({...v,[id]:true}));setMessage('');try{const fn=httpsCallable(functions,'updateOrderOperation');await fn({orderId:id,...data});if(data.status){toast('Status do pedido atualizado.');const order=orders.find(x=>x.id===id);if(order&&messageEnabled&&['paid','preparing','ready','out_for_delivery','completed'].includes(data.status)){setTimeout(()=>sendStatusWhatsapp(order,data.status!),0);}}}catch(e:any){const m=e?.message?.replace('FirebaseError: ','')||'Não foi possível atualizar o pedido.';setMessage(m);toast(m,'error');}finally{setBusyIds(v=>({...v,[id]:false}))}}
 async function confirmCash(id:string){if(busyIds[id])return;setBusyIds(v=>({...v,[id]:true}));setMessage('');try{const fn=httpsCallable(functions,'confirmOfflinePayment');await fn({orderId:id});setMessage('Pagamento confirmado e lançado no caixa aberto, quando houver.');toast('Pagamento confirmado.');}catch(e:any){const m=e?.message?.replace('FirebaseError: ','')||'Não foi possível confirmar o pagamento.';setMessage(m);toast(m,'error');}finally{setBusyIds(v=>({...v,[id]:false}))}}
 ...
 function nextStatus(o:Order):OrderStatus|null{if(o.status==='pending_payment')return o.paymentStatus==='paid'?'paid':null;const i=flow.indexOf(o.status);return i>=0&&i<flow.length-1?flow[i+1]:null;}
 ...
 <div className="order-list">{visible.length===0?<div className="empty-state"><h3>Nenhum pedido para a operação de hoje</h3><p>Novos pedidos aparecerão aqui em tempo real. Pedidos antigos finalizados ficam disponíveis em Relatórios.</p></div>:visible.map(o=><article className={`order-card ${ageMinutes(o)>=30&&!["completed","cancelled"].includes(o.status)?"order-needs-attention":""}`} key={o.id}>
   ...
   {!['pending_payment','cancelled'].includes(o.status)&&<div className="order-progress" aria-label="Progresso do pedido">{flow.map((st,i)=><span key={st} className={i<=stepIndex(o.status)?'done':''} title={labels[st]} />)}</div>}
   ...
   <div className="payment-server-status"><ShieldCheck size={17}/><div><small>Pagamento</small><strong>{payLabels[o.paymentStatus]}</strong></div>{o.paymentMethod==='Dinheiro'&&o.paymentStatus==='pending'&&o.status!=='cancelled'&&<button className="secondary-btn" disabled={busyIds[o.id]} onClick={()=>confirmCash(o.id)}>{busyIds[o.id]?'Processando...':'Confirmar recebimento'}</button>}</div>
   ...
   <div className="order-controls"><label>Status do pedido<select value={o.status} disabled={o.status==='cancelled'||busyIds[o.id]} onChange={e=>update(o.id,{status:e.target.value as OrderStatus})}>{Object.entries(labels).filter(([v])=>v!=='cancelled').map(([v,l])=><option value={v} key={v}>{l}</option>)}</select></label>{nextStatus(o)&&<button className="primary-btn quick-status" disabled={busyIds[o.id]} onClick={()=>update(o.id,{status:nextStatus(o)!})}>Avançar para {labels[nextStatus(o)!]} <ArrowRight size={16}/></button>}{messageEnabled&&['paid','preparing','ready','out_for_delivery','completed'].includes(o.status)&&<button className="secondary-btn" type="button" onClick={()=>sendStatusWhatsapp(o,o.status)}><MessageCircle size={17}/>Enviar WhatsApp</button>}{o.status!=='cancelled'&&<button className="secondary-btn danger-action" disabled={busyIds[o.id]} onClick={()=>cancel(o.id)}><XCircle size={17}/>Cancelar pedido</button>}</div>
 </article>)}</div></>} </>;
}